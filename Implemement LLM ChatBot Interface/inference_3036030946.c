...
$ gcc -o inference inference_3036030946.c -O3 -lm
or with Makefile (recommended)
$ make -B inference
...
pid_t parent_id; //Initialise variable for parent_id storage

//Handler for SIGINT signal.
void sigint_handler(int signum){
	kill(parent_id, SIGUSR2); //Send SIGUSR2 signal to parent
...
int main(int argc, char *argv[]) {
    ...
    parent_id = getppid();//get parent pid
    signal(SIGINT, sigint_handler); //Direct SIGINT signal to custom handler
    rng_seed = atoi(argv[1]); //convert seed argument to integer
    ...
    while (num_prompt < 4) {
        prompts[num_prompt] = (char *)malloc(MAX_PROMPT_LEN * sizeof(char)); //allocate memory for the prompts
        if (fgets(prompts[num_prompt], MAX_PROMPT_LEN , stdin) !=NULL) { //if no error in reading stdin, then:
            generate(prompts[num_prompt]); //generate tokens
            kill(parent_id, SIGUSR1); //inform the parent that you are done using SIGUSR1
            num_prompt++; //Increment numprompt
        }
    } 
